...
<!DOCTYPE HTML>
<head>
<title>ORGANIC VEGETABLE MARKET</title>
...
$sqlDetail = " SELECT  prm_ID, prm_Name, prm_Path, prd_Qty, prd_Prize, prd_Unit FROM  tb_pdtmaster A
INNER JOIN tb_pdtdetail B on B.prd_prm_ID  = A.prm_ID
WHERE Status = 'Y' and prm_ID='$id'
group By prm_ID
 ";
...
if($result){
  ...
  $fieldinfo=mysqli_fetch_row($result);
    ...
    $a = 1;
   echo " <h2>$fieldinfo[1] </h2>
					 <div class='price'>
						
						 <img src='$fieldinfo[2]' alt='' />
					</div>
 ...
 <p>Number of units :</p>
 <input type='number'id='qty' class='text_box' type='text' name='qty'
 							 value='1' min='1' />	
 								  
 							  <input type='hidden'  name='id' value='$id' min='1' />	
 							  <input type='hidden'  name='prize' value='$fieldinfo[4]' min='1' />	


									<p>Unit Price: <span>Rs.$fieldinfo[4]/$fieldinfo[5]</span></p>
									<br/>
					</div>
